...
<body>
  <div class="cert">
    ...
    <div class="cert-footer">
      <span class="cert-footer-text">TRACE PROTOCOL · SUI TESTNET · WALRUS STORAGE · MEMWAL COLLECTIVE MEMORY BANK</span>
      <span class="cert-footer-text">GENERATED ${new Date().toUTCString()}</span>
    </div>

    ${data.bankSightingCount !== undefined ? `
    <!-- Collective Memory Bank Summary (F-9) -->
    <div style="margin-top:24px;background:#0a0a0a;border:1px solid #1a1a1a;border-radius:12px;padding:20px;font-family:monospace;">
      <div style="color:#10b981;font-size:11px;letter-spacing:2px;margin-bottom:12px;">COLLECTIVE MEMORY BANK</div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;">
        <div>
          <div style="color:#6b7280;font-size:10px;">TOTAL SIGHTINGS</div>
          <div style="color:#f9fafb;font-size:24px;font-weight:bold;">${data.bankSightingCount}</div>
        </div>
        <div>
          <div style="color:#6b7280;font-size:10px;">FIRST SEEN</div>
          <div style="color:#f9fafb;font-size:11px;margin-top:6px;">${data.bankFirstSeen ?? "Unknown"}</div>
        </div>
        <div>
          <div style="color:#6b7280;font-size:10px;">SOURCES</div>
          <div style="color:#f9fafb;font-size:11px;margin-top:6px;">${(data.bankSources ?? []).join(", ") || "Unknown"}</div>
        </div>
      </div>
      <div style="margin-top:12px;color:#6b7280;font-size:10px;">
        Sighting records stored permanently on Walrus via MemWal · Privacy-preserving via Seal anonymization
      </div>
    </div>` : ""}
  </div>
</body>
