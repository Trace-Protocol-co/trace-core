...
export function getSuiClient() {
    if (!_client)
        _client = new SuiClient({ url: CONFIG.SUI_RPC, network: CONFIG.SUI_NETWORK });
    return _client;
}
// ============================================================================
...
export function computePerceptualHash(bytes) {
    const digest = crypto.createHash("sha256").update(bytes).digest();
    const pHash = new Uint8Array(16);
    for (let i = 0; i < 16; i++)
        pHash[i] = digest[i] ^ digest[i + 16];
    return pHash;
}
export async function signBytes(bytes, keypair) {
...
// Walrus Integration
// ============================================================================
export async function uploadToWalrus(blob) {
...
// Sign & Broadcast
// ============================================================================
export async function signAndBroadcast(tx, keypair) {
...
// Telemetry Parser
// ============================================================================
export async function parseTelemetry(digest, _createdObjectIds) {
    const client = getSuiClient();
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
        try {
            ...
            if (attempt === 4)
                throw new Error(`Transaction not indexed after 5 attempts: ${digest}`);
            await new Promise((r) => setTimeout(r, 2000 * (attempt + 1)));
        }
    }
    const changes = (txData.objectChanges ?? []).filter((c) => c.type === "created");
    let mediaId = "";
    let editRecordId;
    let blobId = "";
    for (const change of changes) {
        const t = change.objectType ?? "";
        if (t.includes("::media::MediaRecord"))
            mediaId = change.objectId;
        else if (t.includes("::media::EditRecord"))
            editRecordId = change.objectId;
    }
    for (const ev of txData.events ?? []) {
        const parsed = ev.parsedJson;
        if (parsed && typeof parsed["blob_id"] === "string")
...
// ============================================================================
export async function registerMedia(input, keypair) {
