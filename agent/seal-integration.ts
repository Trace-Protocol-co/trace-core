...
function getSealClient(): SealClient | null {
  ...
  try {
    ...
    console.warn("[Seal] Client init failed:", (e as Error).message?.slice(0, 60));
    return null;
  }
}
 ...
 */
export async function encryptSighting(
  ...
  try {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const id = new TextEncoder().encode(sightingId.slice(0, 32).padEnd(32, "0")) as any;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const data = new TextEncoder().encode(sensitiveJson) as any;

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { encryptedObject } = await (client as any).encrypt({
      threshold:  1,
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const raw = encryptedObject as any;
    const bytes: Uint8Array = raw instanceof Uint8Array ? raw : raw.toBytes ? raw.toBytes() : Buffer.from(JSON.stringify(raw));
    ...
    console.warn("[Seal] Encrypt failed:", (e as Error).message?.slice(0, 80));
    return null;
  }
}
 ...
 */
export function getSealStatus(): object {
  return {
